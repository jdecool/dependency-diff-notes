...
A bot that comments on a merge request with updated dependencies: it inspects a project's dependency state (currently Composer/PHP, read from `composer.lock`), determines what changed, and posts a comment on the corresponding merge request reporting them.
