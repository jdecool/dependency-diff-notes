...
> **Supported ecosystem:** the name is deliberately generic, but the bot currently supports only Composer (PHP), reading from `composer.lock`. Other ecosystems may follow later.
...
_Usage details will be filled in as commands and flags are implemented._
...
Run as a job in a consumer project's merge request pipeline, the bot compares `composer.lock` between the merge-base of the target branch and the current commit (see the description at the top of this README), and creates or updates a single note on the merge request reporting the dependency changes found.

See [`examples/gitlab-ci.yml`](examples/gitlab-ci.yml) for a copy-pasteable job.

Outside of a merge request pipeline (`--request-iid`/`CI_MERGE_REQUEST_IID` empty), the bot prints a message and exits `0` without doing anything — it's safe to leave the job unrestricted by `rules` if you'd rather not gate it on `$CI_PIPELINE_SOURCE`.

### GitHub Actions

Run as a step in a consumer project's pull request workflow, the bot compares `composer.lock` between the merge-base of the base branch and the current commit, and creates or updates a single comment on the pull request reporting the dependency changes found.
...
| `--target-branch`      | Change Request target branch      | `CI_MERGE_REQUEST_TARGET_BRANCH_NAME`    | `GITHUB_BASE_REF`              | none (required in Change Request context) |
| `--token`              | Forge API token                   | `DEPENDENCY_DIFF_NOTES_TOKEN`              | `GITHUB_TOKEN`                 | none (required in Change Request context) |
| `--composer-lock-path` | Path to `composer.lock`           | `DEPENDENCY_DIFF_NOTES_LOCK_PATH`          | `DEPENDENCY_DIFF_NOTES_LOCK_PATH` | `composer.lock` |
...
`--composer-lock-path` and `--repo-dir` always have safe defaults and can be left unset in most setups.
